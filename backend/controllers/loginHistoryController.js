...
exports.deleteByDate = async (req, res) => {
  ...
  if (!date) {
    return res.status(400).json({ error: 'Missing date parameter. Expected format YYYY-MM-DD' });
  }
  ...
  try {
    ...
    const deletedCount = await LoginHistory.destroy({
      where: {
        time: {
          [Op.between]: [startOfDay, endOfDay]
        }
    ...
    res.json({ message: `Deleted ${deletedCount} login history records for ${date}` });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};